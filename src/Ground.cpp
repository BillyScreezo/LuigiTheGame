...
Ground::Ground(int x1, int x2, int y1, int y2, size_t type, Map& map)
{
	...
	case 5:
		this->sym = '=';
		if ((int)(this->fx2 - this->fx1) % 2 != 0) this->fx2+=1.0f;
		if ((this->y2 - this->y1) % 2 != 0) this->fy2+=1.0f;
		break;
	case 6:
		this->sym = '^';
		this->fx1 = (float)x1;
...
void Ground::putOnMap()
{
	...
	if (y2 > map->getHeight() - 1) end_y = map->getHeight() - 1;
	else end_y = y2;
	if (x2 > map->getWidth() - 1) end_x = map->getWidth() - 1;
	else end_x = x2;
	if (x1 < 0) start_x = 0;
	if (x1 < map->getWidth())
	{
...
void Ground::setCoord(float x1, float x2, float y1, float y2)
{
	...
	{
		if ((int)(this->fx2 - this->fx1) % 2 != 0) this->fx2 += 1.0f;
		if ((this->y2 - this->y1) % 2 != 0) this->y2++;
	}
	this->x1 = (int)round(fx1);
	this->x2 = (int)round(fx2);
	this->fy1 = y1;
...
int Ground::getSX()
{
	if (type == 9) return (int)round(this->startx);
	else return -12345;
}
int Ground::getEX()
{
	if (type == 9) return (int)round(this->endx);
	else return -12345;
}
int Ground::getSY()
{
	if (type == 9) return (int)round(this->starty);
	else return -12345;
}
int Ground::getEY()
{
	if (type == 9) return (int)round(this->endy);
	else return -12345;
}
