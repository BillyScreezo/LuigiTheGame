...
void control(Map& map, Player& player, std::vector<Ground*>& ground_list, std::vector<Enemy*>& ENEMY, bool& isend, bool& isstoryend)
{
	...
	if (!player.isDead())
	{
		if(player.ctrl)
		{
			if (GetKeyState(VK_ESCAPE) < 0) pauseMenu(isend, isstoryend);
			if (GetKeyState('A') < 0 && player.ismove_l)
			{
				player.side = false;
				if (player.ix >= 0) player.x -= player.wspeed;
			...
			if (GetKeyState(VK_LSHIFT) < 0)
			{
				...
				if ((clock() - delay >= 300)&&(player.getLevel() == 3))
				{
					...
					{
						++count;
						if(player.getLevel() == 3) enemyModule(ENEMY, player, ground_list, map, WorkType::PTHROW);
					}
					else
					{
						if (clock() - delay >= 1000)
...
void playLevel(std::vector<Ground*>& ground_list, std::vector<Enemy*>& enemy_list, Map& map, const unsigned int endTime, Player& player, bool& isstoryend)
{
	...
	if (player.isDead()) player.resetPlayer();
	...
	for (size_t i = 0; i < ground_list.size(); i++)
	{
		...
		if (ground_list[i]->getType() == 8 && ground_list[i]->mode == true)
		{
			for (size_t j = 0; j < ground_list.size(); j++)
			{
				...
				{
					if(ground_list[j]->x1 != ground_list[j]->x2) ground_list[j]->fx2 -= bridgespeed;
					else
					{
		...
		if (ground_list[i]->getType() == 9)
		{
			...
			if (ground_list[i]->x1 == ground_list[i]->getSX()) ground_list[i]->side = false;
			if (ground_list[i]->x1 == ground_list[i]->getEX()) ground_list[i]->side = true;
			for (int j = 0; j < static_cast<int>(ground_list.size()); j++)
				if ((ground_list[i]->x2 == ground_list[j]->x1 && 
					ground_list[i]->y1 >= ground_list[j]->y1 && ground_list[i]->y2 < ground_list[j]->y2) || (ground_list[i]->x1 - 1 == ground_list[j]->x2 &&
						ground_list[i]->y1 >= ground_list[j]->y1 && ground_list[i]->y2 < ground_list[j]->y2))
						ground_list[i]->side = !ground_list[i]->side;
			if (player.onground && player.iy + player.height == ground_list[i]->y1 && ((player.ix >= ground_list[i]->x1 && player.ix < ground_list[i]->x2) ||
				(player.ix + player.width >= ground_list[i]->x1 && player.ix + player.width < ground_list[i]->x2)))
			{
		...
		if (ground_list[i]->getType() == 10)
		{
			...
			if (player.onground && player.iy + player.height == ground_list[i]->y1 && ((player.ix >= ground_list[i]->x1 && player.ix < ground_list[i]->x2) ||
				(player.ix + player.width >= ground_list[i]->x1 && player.ix + player.width < ground_list[i]->x2)))
			{
				if (ground_list[i]->side) player.y += blockspeedy;
				else player.y -= blockspeedy;
			}
		}
	}
}
